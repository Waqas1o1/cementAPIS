    ...
    def save(self, *args, **kwargs):
        ...
        if self.id == None:
            ...
            if self.orderStatus == 'Received':
                ...
                fu = FriegthUnloadingLeadger(transaction_type='Debit', description=self.Destination, friegth=self.Frieght *
                                             self.qty, unloading=self.unloading*self.qty, total_amount=(self.Frieght*self.qty)+(self.unloading*self.qty))
                fu.save()
                self.fu_lg_id = fu.id
            ...
            if self.orderStatus == 'Received' and self.company_lg_id == None:
                ...
                fu.total_amount = (self.Frieght*self.qty) + \
                ...
                dispatches = DispatchEnventory.objects.filter(
                    slug=self.slug).values('qty')
                dispatch_qty_sum = 0
                for i in dispatches:
    ...
    def save(self, *args, **kwargs):
        if self.orderPlacement:
            ...
            if self.id == None:
                ...
                p = PartyLeadger(party=self.party, qty=self.qty, totalFrieght=self.friegth, totalUnloading=self.unloading,
                                 transaction_type='Credit', total_amount=(self.qty*self.friegth)+(self.unloading * self.qty))
                p.save()
        ...
        if self.id != None:
            if self.orderPlacement:
                ...
                p.total_amount = (self.qty*self.friegth) + \
                    (self.unloading * self.qty)
                p.save()
...
class DispatchEnventory(models.Model):
    ...
    driver = models.ForeignKey(
        Driver, on_delete=models.CASCADE, default='', null=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.id == None:
            ...
            d = DriverLeadger(driver=self.driver, description=self.destination, transaction_type='Debit', qty=self.qty,
                              friegth=total_frieght, unloading=total_unloading, total_amount=total_frieght+total_unloading)
            d.save()
            self.driver_lg_id = d.id
    ...
    def save(self, *args, **kwargs):
        ...
        if self.id == None:
            ...
            if self.payment_method == 'Bank':
                ...
                cl = CashLeadger(description=self.description,
                                 total_amount=self.recived_amount, transaction_type='Credit')
                cl.save()
                self.cash_lg_id = cl.id
            elif self.payment_method == 'Party':
...
class PayToBank(models.Model):
    bank = models.ForeignKey(
        Bank, on_delete=models.CASCADE)
    bank_2 = models.ForeignKey(
        Bank, on_delete=models.CASCADE, blank=True, null=True, related_name='Bank_2')
    agent = models.ForeignKey(
        Agent, on_delete=models.CASCADE, blank=True, null=True)
    party = models.ForeignKey(
        Party, on_delete=models.CASCADE, null=True, blank=True)
...
class PayToAgent(models.Model):
    ...
    agent_2 = models.ForeignKey(
        Agent, on_delete=models.CASCADE, blank=True, null=True, related_name='agent2')
    party = models.ForeignKey(
        Party, on_delete=models.CASCADE, null=True, blank=True)
    bank = models.ForeignKey(
        Bank, on_delete=models.CASCADE, null=True, blank=True)
